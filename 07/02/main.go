...
func main() {
	...
	shinygold := findBag("shiny gold")

	totalcount = countBags(shinygold, 1) - 1
...
func countBags(b *bag, numbags int) int {
	var total int
	...
	for _, subbag := range b.content {
		subb := findBag(subbag.name)
		total += countBags(subb, subbag.count)
	}

	return numbags + (numbags * total)
